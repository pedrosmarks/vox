...
@Component({
  selector: 'app-eventos', standalone: true, imports: [CommonModule, FormsModule, NavbarComponent],
...
export class EventosComponent implements OnInit {
  ...
  form = { title: '', categoryId: '', description: '', price: '', startDate: '', endDate: '', location: '' };
  ...
  newEvent(): void { this.editingId = null; this.form = { title: '', categoryId: '', description: '', price: '', startDate: '', endDate: '', location: '' }; this.formError = ''; this.formOpen = true; }
  editEvent(event: CivicEvent, click: MouseEvent): void { click.stopPropagation(); this.editingId = event.id; this.form = { title: event.title, categoryId: String(event.categoryId), description: event.description ?? '', price: event.price?.toString() ?? '', startDate: event.startDate?.slice(0, 16) ?? '', endDate: event.endDate?.slice(0, 16) ?? '', location: event.location ?? '' }; this.formError = ''; this.formOpen = true; }
  saveEvent(): void {
    if (!this.form.title.trim() || !this.form.categoryId) { this.formError = 'Título e categoria são obrigatórios.'; return; }
